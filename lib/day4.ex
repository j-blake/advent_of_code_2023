defmodule Day4 do
  ...
  me with something quick, I'll let you borrow my boat and you can go visit the gardener. I got all these scratchcards
  ...
  opaque covering already scratched off. Picking one up, it looks like each card has two lists of numbers separated by a
  ...
  def part_1(path \\ "day4/sample.txt") do
  ...
  def part_2(path \\ "day4/sample.txt") do
    input =
      FileHelper.read_file(path)
      |> parse_input()
    ...
    |> Enum.map(&find_winning_numbers/1)
    |> Enum.with_index(1)
    # create list of tuples {card number, number of copies of card}
    |> Enum.map(fn {map, i} -> {i, MapSet.size(map)} end)
    # initialize a map where keys are card numbers and number of copies is 1
    # iex> assert Day4.init_card_copies_map([7, 4, 12]) === %{1 => 1, 2 => 1, 3 => 1}
    |> Enum.reduce(init_card_copies_map(input), fn
      {card_num, matches}, card_copies_map when matches > 0 ->
        # add new copies to the proceeding cards from the one being inspected
        (card_num + 1)..(card_num + matches)
        |> Enum.reduce(card_copies_map, fn next_card, card_copies_map ->
          # number of copies of card being inspected
          number_of_cards = Map.get(card_copies_map, card_num, 0)
          # add number of copies of card being inspected the proceeding copies
          Map.update(card_copies_map, next_card, 0, fn count -> count + number_of_cards end)
        end)

      _, card_copies_map ->
        card_copies_map
    end)
    |> Enum.map(fn {_card, copies} -> copies end)
    |> Enum.sum()
  end

  defp init_card_copies_map(input) do
    Map.new(1..length(input) |> Enum.map(fn x -> {x, 1} end))
  end

  defp parse_input(input) do
    input
    |> Enum.map(fn row ->
      [card, winning, yours] = String.split(row, [":", "|"])

      %{
        card: card |> String.split() |> List.last() |> String.to_integer(),
        winning_numbers:
          String.split(winning, " ", trim: true) |> Enum.map(fn x -> String.to_integer(x) end),
        chosen_numbers:
          String.split(yours, " ", trim: true) |> Enum.map(fn x -> String.to_integer(x) end)
      }
    end)
  end
  ...
  defp find_winning_numbers(card) do
    ...
    MapSet.intersection(winning, chosen)
  end

  defp score(winning_numbers) do
    winning_numbers
    |> Enum.map(fn map -> MapSet.size(map) end)
